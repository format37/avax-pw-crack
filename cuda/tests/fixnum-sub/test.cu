...
// This is our mock big number subtraction function
__device__ void bn_sub(fixnum *result, fixnum *a, fixnum *b) {
    fixnum::sub(*result, *a, *b);
}

// Kernel function to perform the subtraction
__global__ void sub_kernel(fixnum *result, fixnum *a, fixnum *b) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < fixnum::SLOT_WIDTH) {
        bn_sub(&result[idx], &a[idx], &b[idx]);
    }
}
...
int main() {
    ...
    // Launch the kernel
    sub_kernel<<<1, fixnum::SLOT_WIDTH>>>((fixnum*)result->get_ptr(), (fixnum*)a->get_ptr(), (fixnum*)b->get_ptr());
