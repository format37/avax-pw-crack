...
__device__ void run_add_quick_test(const struct add_quick_test_case* test) {
    BIGNUM_CUDA a, b, n, r;
...
__global__ void test_kernel() {
    // Define test cases
    struct add_quick_test_case test_cases[] = {
    ...
    for (size_t i = 0; i < num_tests; i++) {
        printf("\n=== Test case %zu ===\n", i + 1);
        run_add_quick_test(&test_cases[i]);
    }
}

// Host function to launch the test
void run_bn_mod_add_quick_test() {
    test_kernel<<<1, 1>>>();
