...
bool loadBigNumbers(const char *filename, std::vector<BigNumber> &Px, std::vector<BigNumber> &Py,
    ...
    std::string p1x, p1y, q1x, q1y, addRx, addRy, dblRx, dblRy;
    while (file >> p1x >> p1y >> q1x >> q1y >> addRx >> addRy >> dblRx >> dblRy) {
...
// CUDA kernel to perform point addition and doubling
__global__ void pointOperationsKernel(const BigNumber *Px, const BigNumber *Py, const BigNumber *Qx, const BigNumber *Qy,
...
int main() {
    ...
    pointOperationsKernel<<<(numElements + 255) / 256, 256>>>(d_Px, d_Py, d_Qx, d_Qy,
    ...
    for (int i = 0; i < numElements; ++i) {
        printBigNumber(h_Px[i]); printf(" ");
        printBigNumber(h_Py[i]); printf(" ");
        printBigNumber(h_Qx[i]); printf(" ");
