 ...
 * and point doubling using OpenSSL in Jacobian coordinates.
 * The curve used for operations is secp256k1.
 *
...
int main() {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (sscanf(line, "%64s %64s %64s %64s %64s %64s %64s %64s",
                   x1_hex, y1_hex, x2_hex, y2_hex,
                   rx_hex, ry_hex, rdx_hex, rdy_hex) != 8) {
            fprintf(stderr, "Error parsing line: %s\n", line);
            continue;
        }

        // Convert parsed hex strings to BIGNUM
        ...
        BIGNUM *expected_rx = NULL, *expected_ry = NULL;
        BIGNUM *expected_rdx = NULL, *expected_rdy = NULL;
        ...
        // Perform addition and doubling in Jacobian coordinates
