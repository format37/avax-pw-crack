...
void point_add(Point *R, const Point *P, const Point *Q) {
    ...
    // Special case: if P is the point at infinity, R = Q
    if (is_point_at_infinity(P)) {
        *R = *Q;
        return;
    }
    // Special case: if Q is the point at infinity, R = P
    ...
    // Calculate lambda
    if (!points_equal(P, Q)) {
        // Lambda for point addition
        sub256(numerator, Q->y, P->y);  // Q.y - P.y
        sub256(denominator, Q->x, P->x);  // Q.x - P.x
    } else {
        // To be implemented: Calculate lambda for point doubling
    }

    // lambda = numerator / denominator
    // Implement field division here using extended Euclidean algorithm or Fermat's Little Theorem
...
void point_double(Point *R, const Point *P) {
    ...
    // Special case handling: if P is point at infinity, return point at infinity
    if (is_point_at_infinity(P)) {
        memset(R->x, 0, 32);
        memset(R->y, 0, 32);
        return;
    ...
    // Step 1: Calculate numerator = 3 * x^2
    mul256(temp, P->x, P->x);  // x^2
    add256(temp, temp, temp);  // 2 * x^2
    add256(numerator, temp, P->x);  // 3 * x^2

    // Step 2: Calculate denominator = 2 * y
    add256(denominator, P->y, P->y);

    // Step 3: Calculate lambda = numerator / denominator
    // Note: Implement field division here (e.g., using Fermat's Little Theorem or extended Euclidean algorithm)
    uint8_t denominator_inv[32];
    inv_mod_p(denominator_inv, denominator);  // Find multiplicative inverse of denominator
    mul256(lambda, numerator, denominator_inv);  // lambda = numerator * denominator_inv mod p

    // Calculate the coordinates of the result point R = 2 * P

    // Step 1: x_R = lambda^2 - 2 * x
    mul256(temp, lambda, lambda);  // lambda^2
    add256(temp, P->x, P->x);  // 2 * x
    sub256(R->x, temp, P->x);  // lambda^2 - 2 * x

    // Step 2: y_R = lambda * (x - x_R) - y
    sub256(temp, P->x, R->x);  // x - x_R
    mul256(temp, lambda, temp);  // lambda * (x - x_R)
    sub256(R->y, temp, P->y);  // lambda * (x - x_R) - y
}
